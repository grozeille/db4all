...
public class TableDataResource {
    ...
    public ResponseEntity<Void> replaceData(
    ...
    @CrossOrigin
    @RequestMapping(value = "/{project}/table/{table}/data", method = RequestMethod.PATCH)
    public ResponseEntity<Void> updateData(
        ...
        if(data == null) {
            ...
            for(Map<String, Object> row : tableData) {
                // if there's no Row ID, the row is new, add it. Else, replace the existing row
                Object rowIdValue = row.get(COL_ROW_ID);
                if(rowIdValue == null) {
                    row.put(COL_ROW_ID, UUID.randomUUID().toString());
                    ...
                    boolean found = false;
                    for(Map<String, Object> existingRow : data.getData()) {
                        // if the row is found in the table, replace it
                        Object existingRowId = existingRow.get(COL_ROW_ID);
                        if(rowIdValue.equals(existingRow)) {
                            // replace all columns of the row
                            existingRow.clear();
                            for(Map.Entry<String, Object> col : row.entrySet()) {
                                existingRow.put(col.getKey(), col.getValue());
                            }
                            found = true;
                            break;
                        }
    ...
    @CrossOrigin
    @RequestMapping(value = "/{project}/table/{table}/data", method = RequestMethod.DELETE)
    public ResponseEntity<Void> delete(
        ...
        paraClient.deleteAll(Arrays.asList(t.getId()+ TABLE_DATA_ID_SUFFIX));
